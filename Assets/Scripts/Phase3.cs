...
public class Phase3 : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (Time.time - t > 3) {
            ...
            if (ReadSelectedUser.Instance.sortscorenames.Count > current) {
                ...
                name.GetComponent<Text>().text = ReadSelectedUser.Instance.sortscorenames[current];
                name.transform.parent.Find("score").GetComponent<Text>().text = ReadSelectedUser.Instance.sortscorepoints[current].ToString();
                name.transform.parent.Find("Ranking").GetComponent<Text>().text = (current + 1).ToString();
                float max = (float)ReadSelectedUser.Instance.sortscorepoints[0];
                float now = (float)ReadSelectedUser.Instance.sortscorepoints[current];
                name.transform.parent.Find("bar").transform.localScale = new Vector3(now / max, 1.0F, 0);
                current = current + 9;
            }
            else
            {
                if ( int.Parse(transform.parent.name)-4<(ReadSelectedUser.Instance.sortscorenames.Count-3)%9) {
                    current = int.Parse(transform.parent.name) - 1;
                    name.GetComponent<Text>().text = ReadSelectedUser.Instance.sortscorenames[current];
                    name.transform.parent.Find("score").GetComponent<Text>().text = ReadSelectedUser.Instance.sortscorepoints[current].ToString();
                    name.transform.parent.Find("Ranking").GetComponent<Text>().text = (current + 1).ToString();
